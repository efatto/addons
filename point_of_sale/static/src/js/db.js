function openerp_pos_db(instance, module){
    ...
    module.PosLS = instance.web.Class.extend({
        ...
        init: function(options){
            ...
            this.product_by_lot_ref = {}; // 20150423 - EAN13 LOT SUPPORT
            this.lot_id_by_lot_ref = {}; // 20150423 - EAN13 LOT SUPPORT
        ...
        add_lots: function(lots){
            ...
            for(var i = 0, len = lots.length; i < len; i++){
                var lot = lots[i];
                this.product_by_lot_ref[lot.ref] = lot.product_id;
                this.lot_id_by_lot_ref[lot.ref] = lot.id;
            }
        },
        ...
        // 20150423 - EAN13 LOT SUPPORT
        get_product_by_lot_ean13: function(ean13){
            var key = ean13.substring(0,12);
            if(this.product_by_lot_ref[key]){
                var product = this.product_by_lot_ref[key];
                var lot_id = this.lot_id_by_lot_ref[key];
                return {'data': this.product_by_id[product[0]], 'lot_id': lot_id}
            }
            return undefined;
        },
